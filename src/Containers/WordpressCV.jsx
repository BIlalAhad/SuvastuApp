...
export default function WordpressCV() {
  const Firebase = UseFirebase()
  const [data, setData] = useState([])
  useEffect(() => {
    Firebase.listAllCV().then((item) => {
      setData(item.docs)
    })
  }, [])
  return (
        ...
        <section className='w-full m-5'>
          <div class='max-w-screen-xl mx-auto px-5 bg-white min-h-sceen h-[80vh]'>
            <div class='flex flex-col items-center'>
              <h2 class='font-bold text-5xl mt-5 tracking-tight'>Worldpress</h2>
              <p class='text-neutral-500 text-xl mt-3'>CV's</p>
            </div>
            <div class='grid divide-y divide-neutral-200 max-w-xl mx-auto mt-8 '>
              {data.map((item) => {
                if (item.data().skills == 'wordpress') {
                  return (
                    <div class='py-5'>
                      <details class='group'>
