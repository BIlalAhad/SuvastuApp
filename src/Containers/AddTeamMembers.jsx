...
export default function AddTeamMembers() {
    ...
    useEffect(()=>{
        Firebase.listAllMembers().then((item)=>{setMember(item.docs)})
    },[member])
  return (
    <>
        ...
        <>
            ...
            <div className="relative w-auto my-6 mx-auto max-w-3xl">
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <form className=' space-y-2 text-center max-w-lg bg-gray-100 shadow-2xl p-4' >
                        <input type="text" className='w-full p-2 border-2' value={employname} name='employname' placeholder='Employ Name'  onChange={(e)=>setEmployname(e.target.value)}/>
                        <input type="text" className='w-full p-2 border-2' value={employrank} name='employRank' placeholder='Employ Rank' onChange={(e)=>setEmployrank(e.target.value)} />
                        <input type="email" className='w-full p-2 border-2' value={employemail} name='employEmail' placeholder='Employ email' onChange={(e)=>setEmployemail(e.target.value)} />
                        <input type="file" className='w-full p-2 border-2' name='employimg' placeholder='Employ email' onChange={(e)=>setEmployimg(e.target.files[0])} />
          ...
          <div className="opacity-25 fixed inset-0 z-40 bg-black"></div>
                ...
                <section className='sm:m-9 mt-20'>
                    <div className='text-3xl sm:text-4xl flex justify-end  p-2' onClick={()=>setShowModal(true)}><BsPersonPlusFill/></div>
                    <table className='sm:w-full'>
                        <thead>
                            <tr className=' bg-gray-100 text-left'>
                                <th className='p-2 border-b-2'> <img src="" alt="" /></th>
                                <th className='p-2 border-b-2'> Name</th>
                                <th className='p-2 border-b-2'> Email</th>
                                <th className='p-2 border-b-2'> Rank</th>
                                <td><button className='text-2xl text-red-700'><RiDeleteBin5Line/></button></td>
                            </tr>
                        </thead>
                        <tbody>
                            {
                                    ...
                                    return(
                                        <>
                                            <tr className='border-b-2'>
                                                <td><img className='w-20 h-16 object-cover p-1' src={`https://firebasestorage.googleapis.com/v0/b/suvastutech-1e3c4.appspot.com/o/${member.data().imageURL}?alt=media&token=b992de0a-10fa-461b-9284-a8a5762785a1` }  alt="" /></td>
                                                <td>{member.data().employname}</td>
                                                <td>{member.data().employemail}</td>
                                                <td>{member.data().employrank}</td>
                                                <td><button className='text-2xl text-red-700' onClick={()=>Firebase.deleteEmploy(member.id)}><RiDeleteBin5Line/></button></td>
                                            </tr>
                                        </>
                                    )
                                })
